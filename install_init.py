...
# Try official pre-built wheels first
    try:
        if system_info.get('metal', False):
            print("Installing llama-cpp-python with Metal support for Apple Silicon")
            install_package("llama-cpp-python", "--extra-index-url https://abetlen.github.io/llama-cpp-python/whl/metal")
            return True
        elif system_info['gpu']:
            if system_info.get('cuda_version'):
                cuda_ver = system_info['cuda_version'].lower()
                if cuda_ver in ['cu121', 'cu122', 'cu123', 'cu124', 'cu125']:
                    print(f"Installing llama-cpp-python with {cuda_ver} support")
                    install_package("llama-cpp-python", f"--extra-index-url https://abetlen.github.io/llama-cpp-python/whl/{cuda_ver}")
                    return True
            elif system_info.get('rocm_version'):
                print(f"Installing llama-cpp-python with ROCm support")
                install_package("llama-cpp-python[rocm]")
                return True
        else:
            print("Installing llama-cpp-python CPU version")
            install_package("llama-cpp-python", "--extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cpu")
            return True
    except Exception as e:
